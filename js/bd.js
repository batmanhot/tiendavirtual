...
async function PostPedidoCliente(datosenvio){
    ...
    // try {
    //     const response = await fetch('http://localhost:8000/api/v1/DatosPedido/', {
    //          method: 'POST',        
    ...
    //         //const {id, nombres, apellido} = jsonResponse;            
    //         console.log(jsonresponse, "EL POST Se ejecuto Bien!!")
    //     }
    // }catch(error){
    ...
    // try{
        fetch(API_URL_DATOSPEDIDO, opciones)
            .then(response => response.json())
            .then(function(data) {                                    
                    const id_pedido = data.id;
                    SaveDataItemsVentas(id_pedido)
                    console.log("EL Post se ejecuto !!")
            })             
    // }catch(error){
    //           console.log(error,"EL POST Se ejecuto mal!!")
    // }   
...
async function PostPedidoItems(datosenvio){
    ...
    try{
        fetch(API_URL_PEDIDOITEMS, opciones)
            .then(response => response.json())
            .then(json => console.log(json,"Se ejecuto bien el POST!!"))
            .catch(err => console.log(err),"EL POST Se ejecuto mal!!")
    }catch(error){
              console.log(error,"EL POST Se ejecuto mal!!")
    }   
}
